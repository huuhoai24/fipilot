...
class InterviewPlannerAgentTests(unittest.IsolatedAsyncioTestCase):
    async def test_create_plan_uses_candidate_profile_evidence(self):
        ...
        self.assertIn("3 to 5 short phrases", llm_service.prompt)
        self.assertIn("one concise sentence", llm_service.prompt)
        self.assertIn("Do not repeat candidate summaries", llm_service.prompt)
        ...
        self.assertEqual(llm_service.kwargs["operation"], "interview_planning")
