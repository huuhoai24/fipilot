...
Resume extraction reads the PDF text layer with PyMuPDF, assigns stable line
indexes, and sends that evidence to the configured Azure OpenAI deployment.
The runtime does not render PDF pages or require YOLO, OpenCV, ONNX Runtime,
Hugging Face models, or a GPU. Image-only/scanned PDFs are rejected with a clear
OCR-required message because OCR is outside this service.
