...
export async function GET(request: Request) {
  ...
  if (backendUrl === undefined) {
    return Response.json({ detail: "Resume service is not configured" }, { status: 503 });
  }
  ...
  try {
    const response = await fetch(
      `${backendUrl}/api/v1/resumes/latest?client_id=${encodeURIComponent(clientId)}`,
      { cache: "no-store" },
    );
    ...
    return Response.json({ detail: "Resume service is unavailable" }, { status: 502 });
  }
}
