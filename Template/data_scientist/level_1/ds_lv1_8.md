...
### Câu 1
* **Độ khó:** Dễ
* **Câu hỏi:** Trong chuẩn bị đặc trưng dữ liệu danh mục (Categorical Features), kỹ thuật Target Encoding (hoặc Mean Encoding) hoạt động ra sao? Hãy chỉ ra rủi ro kỹ thuật lớn nhất về mặt rò rỉ thông tin dữ liệu (Target Leakage) khi áp dụng kỹ thuật này.
* **expected_key_points:**
  - id: KP1_1
    content: Cơ chế mã hóa dựa trên giá trị trung bình của biến mục tiêu
    keypoint_weight: 0.5
  ...
  - id: KP1_2
    content: Rủi ro rò rỉ thông tin dữ liệu mục tiêu (Target Leakage) gây quá khớp
    keypoint_weight: 0.5
    description: Do sử dụng trực tiếp thông tin của biến mục tiêu để sinh ra đặc trưng đầu vào, nếu một nhóm danh mục có số lượng mẫu quá nhỏ, giá trị trung bình sẽ bị chi phối tuyệt đối bởi các mẫu đó, dẫn đến việc rò rỉ thông tin từ tương lai vào mô hình, gây ra hiện tượng Overfitting nặng nề (mô hình đạt điểm tuyệt đối trên tập Train nhưng sụp đổ trên tập Test).
...
### Câu 5
* **Độ khó:** Trung bình
* **Câu hỏi:** Thuật toán Random Forest xây dựng cụm mô hình dựa trên kỹ thuật Bagging. Hãy giải thích khái niệm Out-of-Bag (OOB) Error và tại sao cơ chế này lại cho phép ta đánh giá hiệu năng mô hình một cách khách quan mà không bắt buộc phải sử dụng đến một tập dữ liệu Validation độc lập?
* **expected_key_points:**
  - id: KP5_1
...
* **expected_key_points:**
  ...
  - id: KP9_2
    ...
    keypoint_weight: 0.3
    description: Để tránh việc chuẩn hóa làm mất đi tính chất phi tuyến đặc thù của hàm kích hoạt (ví dụ ép dữ liệu rơi vào vùng tuyến tính trung tâm của Hàm Sigmoid), Batch Normalization đưa thêm hai tham số có khả năng học được (Trainable parameters) là $\gamma$ (Scale) và $\beta$ (Shift). Công thức cuối cùng: $Y = \gamma \hat{X} + \beta$, cho phép mạng nơ-ron tự động điều chỉnh tối ưu hóa cấu trúc bề mặt dữ liệu.
  - id: KP9_3
