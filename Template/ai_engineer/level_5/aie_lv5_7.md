# Bộ Câu Hỏi Phỏng Vấn AI Engineer (Level 5) - Tập Đề Two-Tower Models và Session-based Rec (7)
...
* **expected_key_points:**
  ...
  - id: KP5_2
    ...
    keypoint_weight: 0.5
    description: Độ tương đồng được tính bằng tích vô hương (dot product) hoặc cosine similarity $u \cdot v^T$. Khi suy luận, các vector Item có thể tính trước (offline) và lưu vào Vector DB. Khi user truy cập, chỉ cần chạy tháp User lấy vector $u$ rồi tìm kiếm lân cận gần nhất (ANN) trên DB để trả kết quả dưới 10ms.
