...
### Câu 1
* **Độ khó:** Dễ
* **Câu hỏi:** Trong quy trình CI/CD và quản lý mã nguồn, khi cần tích hợp thay đổi từ nhánh tính năng (Feature Branch) vào nhánh chính (Main Branch), hãy phân biệt sự khác biệt cơ bản về mặt cơ chế hoạt động và cấu trúc lịch sử commit (Git History) giữa hai câu lệnh: `git merge` và `git rebase`.
* **expected_key_points:**
  - id: KP1_1
...
* **expected_key_points:**
  ...
  - id: KP8_3
    content: Phân tích khả năng chịu lỗi tối ưu của cấu hình số lẻ so với số chẵn liền kề
    keypoint_weight: 0.2
...
* **expected_key_points:**
  ...
  - id: KP10_3
    ...
    keypoint_weight: 0.2
    description: Trong quá trình dịch chuyển cuốn chiếu, Kubernetes tạo Pod