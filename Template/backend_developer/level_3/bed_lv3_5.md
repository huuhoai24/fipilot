# Bộ Câu Hỏi Phỏng Vấn Backend Developer (Level 3) - Tập Đề Microservices Patterns và Service Mesh (5)
...
### Câu 1
* **Độ khó:** Dễ
* **Câu hỏi:** Service Mesh là gì? Phân tích vai trò và sự phân cấp giữa Data Plane (Sidecar Proxy) và Control Plane trong kiến trúc Service Mesh (như Istio).
* **expected_key_points:**
  - id: KP1_1
...
### Câu 2
* **Độ khó:** Dễ
* **Câu hỏi:** Giải thích cơ chế hoạt động của Circuit Breaker Pattern. Hãy nêu chi tiết ba trạng thái của nó (Closed, Open, Half-Open) và điều kiện chuyển trạng thái.
* **expected_key_points:**
  - id: KP2_1
...
### Câu 3
* **Độ khó:** Dễ
* **Câu hỏi:** So sánh sự khác nhau về kiến trúc truyền thông, định dạng dữ liệu và hiệu năng giữa giao thức gRPC (HTTP/2) và REST (HTTP/1.1) khi giao tiếp liên dịch vụ (Inter-service communication).
* **expected_key_points:**
  - id: KP3_1
...
### Câu 4
* **Độ khó:** Trung bình
* **Câu hỏi:** Thiết kế giải pháp Phát hiện dịch vụ phân tán (Service Discovery - ví dụ sử dụng Consul) cho hệ thống gồm hàng ngàn instances dịch vụ thay đổi trạng thái liên tục.
* **expected_key_points:**
  - id: KP4_1
...
### Câu 5
* **Độ khó:** Trung bình
* **Câu hỏi:** Thiết kế API Gateway thông minh tích hợp tính năng giới hạn băng thông (Rate Limiting) sử dụng thuật toán Token Bucket kết hợp Redis phân tán.
* **expected_key_points:**
  - id: KP5_1
...
### Câu 6
* **Độ khó:** Trung bình
* **Câu hỏi:** Phân tích cơ chế hoạt động của Giám sát phân tán (Distributed Tracing) sử dụng OpenTelemetry (Traces, Spans, Context Propagation) để định vị lỗi trong hệ thống microservices.
* **expected_key_points:**
  - id: KP6_1
...
### Câu 7
* **Độ khó:** Trung bình
* **Câu hỏi:** So sánh sự khác nhau về kiến trúc và hiệu năng xử lý I/O giữa API Gateway dạng Non-blocking (như Spring Cloud Gateway, Kong) và Blocking (như Netflix Zuul 1).
* **expected_key_points:**
  - id: KP7_1
...
### Câu 8
* **Độ khó:** Khó
* **Câu hỏi:** Thiết kế hệ thống điều phối giao dịch đa dịch vụ (Distributed Transaction Coordination) quy mô lớn đảm bảo tính nhất quán dữ liệu mà không làm giảm đáng kể hiệu năng hệ thống (throughput) bằng cách kết hợp Saga và Outbox pattern.
* **expected_key_points:**
  - id: KP8_1
...
### Câu 9
* **Độ khó:** Khó
* **Câu hỏi:** Thiết kế kiến trúc bảo mật Zero-Trust cho hệ thống microservices phức tạp chạy trên Kubernetes, sử dụng mTLS, SPIFFE/SPIRE và Open Policy Agent (OPA) để kiểm soát truy cập mịn.
* **expected_key_points:**
  - id: KP9_1
...
### Câu 10
* **Độ khó:** Khó
* **Câu hỏi:** Thiết kế giải pháp triển khai phiên bản mới không gián đoạn dịch vụ (Zero-downtime deployment: Canary và Blue-Green) tự động hóa hoàn toàn dựa trên các chỉ số đo lường hiệu năng thời gian thực của Service Mesh.
* **expected_key_points:**
  - id: KP10_1
