# Bộ Câu Hỏi Phỏng Vấn Backend Developer (Level 4) - Tập Đề Apache Kafka và Event-Driven Architecture (3)
...
### Câu 1
* **Độ khó:** Dễ
* **Câu hỏi:** So sánh sự khác nhau về mô hình phân phối tin nhắn, cơ chế lưu trữ và khả năng chịu tải giữa Apache Kafka (Pull-based) và RabbitMQ (Push-based).
* **expected_key_points:**
  - id: KP1_1
  ...
  - id: KP1_2
    content: Khản năng chịu tải và Thứ tự tin nhắn
    keypoint_weight: 0.5
...
### Câu 2
* **Độ khó:** Dễ
* **Câu hỏi:** Giải thích khái niệm Idempotency (tính lũy đẳng) trong xử lý tin nhắn. Tại sao nó lại là yêu cầu bắt buộc khi thiết kế các hệ thống xử lý bất đồng bộ?
* **expected_key_points:**
  - id: KP2_1
...
### Câu 3
* **Độ khó:** Dễ
* **Câu hỏi:** Phân biệt ba cơ chế phân phối tin nhắn trong hệ thống Message Queue: At-least-once, At-most-once, và Exactly-once. Cơ chế nào khó thực hiện nhất?
* **expected_key_points:**
  - id: KP3_1
...
### Câu 4
* **Độ khó:** Trung bình
* **Câu hỏi:** Giải thích cơ chế hoạt động của Consumer Group trong Apache Kafka. Làm thế nào để tăng quy mô xử lý (scale out) mà không làm mất đi tính trật tự của tin nhắn trong một Partition?
* **expected_key_points:**
  - id: KP4_1
...
### Câu 5
* **Độ khó:** Trung bình
* **Câu hỏi:** Thiết kế giải pháp xử lý tin nhắn lỗi sử dụng Dead Letter Queue (DLQ) kết hợp Retry Queue có cơ chế thời gian trễ lũy thừa (Exponential Backoff).
* **expected_key_points:**
  - id: KP5_1
...
### Câu 6
* **Độ khó:** Trung bình
* **Câu hỏi:** Phân tích hiện tượng Rebalance trong Kafka Consumer Group. Nguyên nhân xảy ra là gì và làm thế nào để tối ưu hóa cấu hình tránh ảnh hưởng đến độ trễ hệ thống?
* **expected_key_points:**
  - id: KP6_1
...
### Câu 7
* **Độ khó:** Trung bình
* **Câu hỏi:** So sánh sự khác nhau giữa Message Queue truyền thống (như RabbitMQ) và Event Stream (như Kafka) từ góc độ lưu trữ dữ liệu lịch sử và khả năng tái phát lại sự kiện (Event Replay).
* **expected_key_points:**
  - id: KP7_1
...
### Câu 8
* **Độ khó:** Khó
* **Câu hỏi:** Thiết kế hệ thống thanh toán điện tử (Payment Gateway) đảm bảo xử lý chính xác tuyệt đối các giao dịch tài chính sử dụng kiến trúc Event Sourcing kết hợp CQRS (Command Query Responsibility Segregation).
* **expected_key_points:**
  - id: KP8_1
...
### Câu 9
* **Độ khó:** Khó
* **Câu hỏi:** Thiết kế hệ thống xử lý đơn hàng thời gian thực chịu tải cực cao (100k requests/giây) đảm bảo duy trì tính thứ tự nghiêm ngặt của các sự kiện theo từng đơn hàng cụ thể trên cụm Kafka phân tán.
* **expected_key_points:**
  - id: KP9_1
...
### Câu 10
* **Độ khó:** Khó
* **Câu hỏi:** Thiết kế kiến trúc Event Broker phân tán đa vùng địa lý (Multi-datacenter Kafka) đảm bảo đồng bộ hóa dữ liệu với độ trễ thấp và không làm mất mát tin nhắn (No data loss) khi xảy ra thảm họa đứt kết nối mạng liên lục địa (Cross-WAN partition).
* **expected_key_points:**
  - id: KP10_1
  ...
  - id: KP10_2
    content: Đối phó thảm họa mạng phân mảnh (Network Partition)
    keypoint_weight: 0.5
