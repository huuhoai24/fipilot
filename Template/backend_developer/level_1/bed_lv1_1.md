...
* **expected_key_points:**
  - id: KP1_1
    ...
    keypoint_weight: 0.5
    description: GET dùng để đọc dữ liệu (Read); POST dùng để tạo mới dữ liệu (Create); PUT dùng để cập nhật đè dữ liệu (Update); DELETE dùng để xóa dữ liệu (Delete).
  - id: KP1_2
    content: Xác định chính xác tính chất Idempotent (Tính đồng nhất)
    keypoint_weight: 0.5
    description: Idempotent nghĩa là việc thực hiện một yêu cầu nhiều lần liên tiếp sẽ cho ra cùng một kết quả giống như thực hiện một lần duy nhất. Các phương thức có tính Idempotent là GET, PUT, DELETE. POST KHÔNG có tính chất này vì nhiều request liên tiếp sẽ tạo ra nhiều bản ghi trùng lặp.
...
* **expected_key_points:**
  ...
  - id: KP9_2
    ...
    keypoint_weight: 0.2
    description: Vì môi trường mạng vật lý luôn tiềm ẩn rủi ro lỗi phần cứng, đứt cáp hoặc mất kết nối giữa các node, nên yếu tố P (Partition Tolerance) là bắt buộc phải lựa chọn trong thiết kế hệ thống.
  - id: KP9_3
...
* **expected_key_points:**
  - id: KP10_1
    ...
    keypoint_weight: 0.5
    description: Hoạt động theo mô hình Request-Response trực tiếp. Client gửi yêu cầu lên Server và bắt buộc phải treo luồng tính toán để đợi Server phản hồi kết quả về rồi mới chạy tiếp. Ưu điểm: Đơn giản, biết kết quả ngay. Nhược điểm: Dễ gây nghẽn mạch dây chuyền nếu một dịch vụ phía sau bị chậm (Tight Coupling).
  - id: KP10_2
    content: Cơ chế giao tiếp bất đồng bộ (Asynchronous) của Message Broker
    keypoint_weight: 0.5
    description: Hoạt động theo mô hình Event-driven (Publisher-Subscriber) gián tiếp thông qua hàng đợi (như Kafka, RabbitMQ). Dịch vụ gửi (Producer) chỉ cần đẩy thông điệp vào Broker rồi tiếp tục làm việc khác ngay lập tức; dịch vụ nhận (Consumer) tự động lấy ra xử lý sau khi rảnh. Ưu điểm: Giúp giảm sự phụ thuộc giữa các dịch vụ (Loose Coupling), tăng khả năng chịu tải chống nghẽn. Nhược điểm: Hệ thống phức tạp, khó debug luồng dữ liệu và không nhận được kết quả tức thì.