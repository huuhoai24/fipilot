...
export function SpeechInterviewPage() {
    ...
    const connect = async (forceRefresh = false) => {
      ...
      try {
        ...
        socket.onmessage = (message) => {
          ...
          try {
            ...
            if (event.type === 'connected') {
              ...
              setTransportError('')
              setAudioWarning('')
              // The current question was generated by the REST call, so nothing
              // on this socket has spoken it yet. Ask for it explicitly; this
              // also re-reads the question after a reconnect.
              ...
              // Recoverable: the recogniser fell behind and skipped audio. The
              // interview continues, so this is a notice, not an error.
              setAudioWarning(
        ...
        socket.onclose = (event) => {
          ...
          setQuestionStreaming(false)
          setVoiceState(VoiceInterviewState.IDLE)
          // 1009 is sent when the server rejects an audio payload. Dropping the
          // candidate mid-answer with no recovery was worse than reconnecting and
          // refetching the interview state.
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="hairline flex flex-col gap-4 rounded-[24px] border border-border bg-surface px-6 py-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-accent">
            <Mic className="h-4 w-4 text-accent" />
            Voice Interview
          </div>
          <h1 className="mt-3 font-display text-3xl font-semibold tracking-tight-display text-text-primary">AI Interviewer</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        <main className="space-y-6">
          <section
            className="hairline relative overflow-hidden rounded-[28px] border border-border bg-surface px-6 py-12 text-center shadow-2xl shadow-black/10 sm:px-12 sm:py-16"
            aria-labelledby="current-voice-question"
          >
            <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(101,230,189,.15),transparent_45%)]" aria-hidden="true" />
            <div className="relative mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-accent-soft">
              <Bot className="h-6 w-6 text-accent" aria-hidden="true" />
            </div>
            <h2 id="current-voice-question" className="relative mt-6 text-xs font-semibold uppercase tracking-[0.18em] text-text-faint">
              Current Question
            </h2>
            {isComplete ? (
              <div className="mt-4">
              ...
              <p className="relative mx-auto mt-5 max-w-4xl text-balance font-display text-2xl font-medium leading-9 tracking-tight-display text-text-primary sm:text-3xl sm:leading-10">
                {questionStreaming
                  ? currentQuestionText
          ...
          <section className="hairline rounded-[28px] border border-border bg-surface px-5 py-8 sm:px-10 sm:py-10">
            ...
            {microphoneError && (
              <div
                className="mx-auto mt-4 flex max-w-xl items-start justify-between gap-3 rounded-2xl border border-danger/30 bg-danger/10 p-4"
                role="alert"
              >
            ...
            {audioWarning && (
              <div
                className="mx-auto mt-4 max-w-xl rounded-2xl border border-warning/30 bg-warning/10 p-4"
                role="status"
              >
          ...
          <div className="hairline rounded-[24px] border border-border bg-surface p-6">
            <TranscriptPreview
        ...
        <aside className="space-y-5 lg:sticky lg:top-8 lg:self-start">
          <section className="hairline rounded-[24px] border border-border bg-surface p-6" aria-labelledby="voice-progress-title">
          ...
          <section className="hairline rounded-[24px] border border-border bg-surface p-6" aria-labelledby="voice-candidate-title">
            <div className="flex items-start gap-3">
