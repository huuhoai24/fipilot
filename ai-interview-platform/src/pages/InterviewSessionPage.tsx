...
export function InterviewSessionPage() {
  ...
  const playAudio = async (arrayBuffer: ArrayBuffer) => {
    ...
    try {
      ...
      console.error("Lỗi phát lại âm thanh:", e);
      setIsSpeaking(false);
    }
  };
  ...
  useEffect(() => {
    if (!sessionId) {
      setStatus('Lỗi: Thiếu ID phiên.');
      return;
    }
    const ws = new WebSocket(`${WS_URL}/phỏng vấn/${sessionId}`);
    wsRef.current = ws;
    ...
    ws.onmessage = async (event) => {
      if (typeof event.data === "string") {
        try {
          ...
          console.error("Không thể phân tích tin nhắn văn bản", e);
        }
      } else {
        setIsSpeaking(true);
